...
int lfs_emubd_create(const struct lfs_config *cfg, const char *path) {
    LFS_TRACE("lfs_emubd_create("
                "%p {%p, %"PRIu32", %"PRIu32", %"PRIu32", %"PRIu32"}, \"%s\")",
            (void*)cfg, cfg->context, cfg->read_size, cfg->prog_size,
            cfg->block_size, cfg->block_count, path);
...
void lfs_emubd_destroy(const struct lfs_config *cfg) {
    ...
    lfs_emubd_t *emu = cfg->context;
    free(emu->path);
    LFS_TRACE("lfs_emubd_destroy -> void%s", "");
}
